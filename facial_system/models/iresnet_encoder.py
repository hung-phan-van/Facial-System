...
class IResNetEncoder(ModelBase):
    def __init__(self, checkpoint_path=None, **kwargs):
        super(IResNetEncoder, self).__init__()
        self.checkpoint_path = checkpoint_path
        self.model = IResNet(IBasicBlock, [3, 13, 30, 3], **kwargs)
        self.checkpoint_path = checkpoint_path
    ...
    def load_checkpoint(self, device):
        ...
        print('Loaded embedding model from checkpoint path {}'.format(self.checkpoint_path))
        return True
    ...
    def inference_batch(self, rgb_images, **kwargs):
        ...
        with torch.no_grad():
            embeddings = self.model(batch_ts_img)
            is_tta = kwargs.get('tta')
            if is_tta:
                flipped_ts_img = torch.flip(batch_ts_img, dims=[3])
                flipped_embs = self.model(flipped_ts_img)
                embeddings = (embeddings + flipped_embs) / 2
