...
def load_config():
    return json.load(open('/Data/Hung/dat_retina/retina_face_optimization/detect_config.json','r'))
...
def load_net(x =1280 , y =720 ):
    if network == "mobile0.25":
        ...
        print("Don't support network!")
        exit(0)
    trained_model = "/Data/Hung/dat_retina/retina_face_optimization/weights/RBF_Final.pth"
    device = torch.device("cuda")
    model = load_model(net, trained_model).eval().to(device)
    print('model',model)
    sample = torch.ones((1, 3, x, y)).cuda()
    print('sample', sample)
    model_trt = torch2trt(model, [sample])
