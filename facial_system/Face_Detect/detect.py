...
import onnxruntime
import onnx
session = onnxruntime.InferenceSession('./weights/RFB.onnx',None)
...
parser.add_argument('--i',default='input_im')
...
if __name__ == '__main__':
    ...
    # testing begin
    for im in paths:
        image_path = args.i+'/'+im
        img_raw = cv2.imread(image_path, cv2.IMREAD_COLOR)
        original_size = (img_raw.shape[1],img_raw.shape[0])
        ...
        # img_raw = cv2.resize(img, None, None, fx=resize, fy=resize)

        scale = torch.Tensor([img.shape[1], img.shape[0], img.shape[1], img.shape[0]])
        img -= (104, 117, 123)
        img = img.transpose(2, 0, 1)
        img = img.reshape(1, 3, img.shape[1], img.shape[2]).astype('float32')
        scale = scale.to(device)

        tic = time.time()
        # loc, conf, landms = net(img)  # forward pass
        loc, conf, landms = session.run([], {input_name: img})
        print('Inference time: {:.4f}'.format(time.time() - tic))
        loc = torch.Tensor(loc).to(device)
        conf = torch.Tensor(conf).to(device)
        landms = torch.Tensor(landms).to(device)
        ...
        img_raw = cv2.resize(img_raw, (sz,sz))
        # show image
        if args.save_image:
            for b in dets:
            ...
            img_raw = cv2.resize(img_raw, original_size)
            name = "./output_im/"+im
